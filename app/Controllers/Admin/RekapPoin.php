...
class RekapPoin extends BaseController
{
    public function index()
    {
        $data['grid']   = $this->grid();
        $data['search'] = $this->search();
        $data['title']  = 'Rekap Poin';
    ...
    public function grid()
    {
        ...
        while (strtotime($tanggal_awal) <= strtotime($tanggal_akhir)) {
            ...
            $sub_sql[] = "(select count(*) from barang where bar_kurir_id=kurir.id and bar_tgl='".$tanggal_awal."') ||' / '|| 
            (select count(*) from barang where bar_kurir_id_antar=kurir.id and bar_tanggal_terkirim='".$tanggal_awal."') as h_".str_replace("-","_",$tanggal_awal);
        }
        ...
        return $grid->set_query($SQL)
            ...
            ->configure(
                array(
                    'datasouce_url' => base_url("admin/rekapPoin/grid?datasource&" . get_query_string()),
